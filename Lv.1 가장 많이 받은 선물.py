def solution(friends: list, gifts: list):
    answer = [0] * len(friends)  # 정답 리스트
    index_list = []  # +=1 을 하기 위한 인덱스
    # 준사람/받은사람 리스트, answer 리스트를 0으로 초기화
    _list = [[0] * len(friends) for _ in range(len(friends))]
    ...
    set_index_list = list(set([tuple(sorted(index_list)) for index_list in index_list]))  # 중복제거
